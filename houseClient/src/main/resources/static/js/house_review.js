layui.use(['table', 'jquery'], function () {
    ...
    let retable = table.render({
        ...
        , cols: [
            [
                ...
                , {field: 'ownerid', title: '房东编号', width: 180, hide: true}
                , {field: 'ownername', title: '房东姓名', width: 180}
                , {
                field: 'owneridentity',
                title: '房屋类型',
                width: 180,
                ...
                , {field: 'ownerphone', title: '房屋地址', width: 180, hide: true}
                , {field: 'houseid', title: '房屋编号', width: 180, sort: true}
                , {field: 'housename', title: '房屋名称', width: 180}
                ...
                , {field: 'startdate', title: '签订时间', width: 180, sort: true}
                , {field: 'enddata', title: '到期时间', width: 180, sort: true}
                , {
                title: '房屋详情',
                width: 180,
                templet: '#houser'
                ...
                , {field: 'status', title: '认证状态', width: 120, sort: true, templet: '#checkboxTp2'}
                ,{field: 'define', title: '签订状态', width: 120, sort: true, templet: '#checkboxTp3'}
    ...
    table.on('tool(order)', function (obj) {
        ...
        switch (obj.event) {
            ...
            case 'yesPassed':
                layer.confirm('瓦达西瓦', function (index){
            ...
            case 'checking':
                layer.confirm('可以通过吗' ,{btn:['通过','不通过']}, function (index){
                    ...
                    $.ajax({
                        url:'http://localhost:8080/house/houseCheck',
                        data:{'house_id':data.houseid,'status':1,"access_token": token.access_token},
                        type:'POST',
                        success:function (res){
                    ...
                    $.ajax({
                        url:'http://localhost:8080/house/houseCheck',
                        data:{'house_id':data.houseid,'status':777,"access_token": token.access_token},
                        type:'POST',
                        success:function (res){
                            layer.msg('成功',{icon: 1, time: 1000});
                        },
    ...
    function shouw_contract(data) {
        let $ = layui.jquery;
        const certificate_type = 111;
        let token = get_LocalStorage('TOKEN');
        console.log(data)
...
function show_house(obj, event) {
    layui.use('jquery', function () {
        ...
        window.location.href = '/index/showHouseCheck';
    });
}
